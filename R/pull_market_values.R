...
pull_national_squads <- function(){
  ...
  national_squads <- purrr::map_df(seq(1:6), get_national_squads) %>%
    unique()
  temp_squad_year <- national_squads %>% tidyr::expand(nation,c(2000:2019)) %>% dplyr::rename(year = `c(2000:2019)`) %>%
  ...
  nat_squads_year<-furrr::future_map2_dfr(temp_squad_year %>% dplyr::pull(url),
                                          temp_squad_year %>% dplyr::pull(year),
                                          get_squad_list, .progress = TRUE)
